...
function Neuron(d::Float64)
    ...
    n0 = 0.0          # initial number of open channels
    x = [v0, p0, i0, n0]   # initial state
    N = round(Int64, g/10e-9)

  langevinNeuron(x, g, E, C, d, A, N)
...
function update(neuron, Inject, Δt)
    ...
    neuron.x[2] = p + Δt*(p_infinity - p)/τ + γ(v)*rand(dW,1)[]
...
ax1.plot(t, x[:,1])
ax1.set_title("Langevin Approximation")
